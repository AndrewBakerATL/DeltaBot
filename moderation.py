...
class Moderation:
    ...
    async def kick(self, ctx, user: discord.Member):
        ...
        embed.add_field(name="__**User ID**__", value="**ID:** | {}".format(user.id), inline=True)
        embed.add_field(name="__**User Role**__", value="**Highest:** | {} ".format(user.top_role), inline=True)
        embed.add_field(name="__**Date Joined**__", value="**Joined:** | {}".format(user.joined_at), inline=True)
        embed.add_field(name="__**Action Taken**__", value="***Kicked {} ***".format(user.name), inline=True)
    ...
    async def ban(self, ctx, user: discord.Member):
        ...
        author = ctx.message.author
        channel = ctx.message.channel
        embed = discord.Embed(title="**Banning User**", description="Taking action on {}'s  File From **{} Database**".format(author.mention, ctx.message.server.name), color=0xfc4156)
        embed.set_footer(text="Delta Discipline Report")
        embed.set_author(name="Discipline Report", icon_url="https://cdn.discordapp.com/app-icons/481923206848970803/394817ba790d2fbb9c36715a7ec00576.png")
        embed.add_field(name="__**User Name**__", value="**Name:** | {}".format(user.name), inline=True)
        embed.add_field(name="__**User ID**__", value="**ID:** | {}".format(user.id), inline=True)
        embed.add_field(name="__**User Role**__", value="**Highest:** | {} ".format(user.top_role), inline=True)
        embed.add_field(name="__**Date Joined**__", value="**Joined:** | {}".format(user.joined_at), inline=True)
        embed.add_field(name="__**Action Taken**__", value="***Banned {} ***".format(user.name), inline=True)
        embed.set_thumbnail(url=user.avatar_url)
        await self.bot.send_message(channel, embed=embed)
        await self.bot.send_message(server_log, embed=embed)
